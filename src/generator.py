"""
    ...
    Sources are kept best-ranked first and dropped worst-ranked first when
    the assembled context would exceed the token budget, since the
    retriever already orders them by relevance.

    chunkcmetadata -> reslice corpus -> context block -> prompt ->
    grounded response
"""
...
def _fit_sources(
    ...
    """
        ...
        return:
            ...
            allways keep the first avalaible block even if the budget is
            exceeded so a question with sources never gets an empty context
    """
    blocks: list[str] = []
    used_tokens = 0
